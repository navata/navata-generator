...
const removeFields = (parameters) => parameters.filter((param) => !parameterBlackList.includes(param.name));
...
const convertProperties = (properties, level) => {
    ...
    for (const [name, value] of Object.entries(properties)) {
        if (parameterBlackList.includes(name))
            continue;
        ...
        if (type === "array") {
            ...
            if (value?.items?.$ref) {
                ...
                if (!generatedTypes.has(typeName)) {
                    generatedTypes.set(typeName, `type ${typeName} = ${getTypeDefinitions(value.items.$ref, 1)}`);
                }
                continue;
            }
        }
...
const genericTypes = async ({ swaggerUrl, apiPath, method = "get", fileName, moduleName, }) => {
    ...
    const functionName = (0, generic_api_1.toCamelCase)(`${moduleName}-${fileName}`);
    const typeFunctionName = (0, generic_api_1.capitalizeFirstLetter)(functionName);
